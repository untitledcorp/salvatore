...
ISO9660Filesystem::ISO9660Filesystem(const std::vector<uint8_t>& partitionData) : data(partitionData) {}

void ISO9660Filesystem::detectFilesystem() {
    if (data[0x8001] == 0x43 && data[0x8002] == 0x44 && data[0x8003] == 0x30 && data[0x8004] == 0x31) {
...
void FAT32Filesystem::detectFilesystem() {
    if (data[0x36] == 0x55 && data[0x37] == 0xAA) {
